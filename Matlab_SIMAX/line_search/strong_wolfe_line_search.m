function [alphas,fs,gs,nf] = strong_wolfe_line_search(problem, d, x0, c1, c2)
...
% Modified by H.Kasai on November 1, 2016
    ...
    while true
        ...
        if (fxx > fx0 + c1*alphax*gx0) || ((i > 1) && (fxx >= fxp))
            [alphas,fs,gs,nfz] = Zoom(problem,x0,d,alphap,alphax,fx0,gx0);
            nf = nf+nfz;
            return;
        end
        if abs(gxx) <= -c2*gx0,
        ...
        if gxx >= 0,
            [alphas,fs,gs,nfz] = Zoom(problem,x0,d,alphax,alphap,fx0,gx0);
            nf = nf+nfz;
            return;
        end
...
%%%%%%%
function [alphas,fs,gs,nf] = Zoom(problem,x0,d,alphal,alphah,fx0,gx0)
