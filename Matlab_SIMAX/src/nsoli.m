function [sol, it_hist, ierr, x_hist] = nsoli(x,f,cost,tol, parms)
%% ---- given cost func and f func
...
f0 = feval(f,x);
fnrm = norm(f0);
it_histx(itc+1,1) = cost(x); it_histx(itc+1,2) = 0; it_histx(itc+1,3) = 0;
fnrmo = 1;
...
elseif lmeth == 5
    g = @(p) dirder(x,p,f,f0);
    %--- [etamax, lb, itmax==restart]
    gmparms = [params(1),params(2),params(3)];
    [step, errstep, total_iters, f_evals] = dtgcr(f0, g, x, gmparms);
%
% CG
%
elseif lmeth == 6
    ...
    %--- [etamax, itmax]
    gmparms = [params(1),params(2)];
    [step, errstep, total_iters, f_evals] = dcg(f0, g, x, gmparms);
else
...
function [sol, res, total_iters, f_evals]= dtgcr(f0,f,x,params)  
%% function TGCR
...
function [sol, res, total_iters, f_evals]= dcg(f0,f,x,params) 
%% function CG
%%----------------------------------------------------------------------- 
%%-------------------- initialize 
errtol = params(1)*norm(f0);
