function [w, infos] = ncg(w, problem, options)
...
%     end 
    ...
    while (optgap > tol_optgap) && (gnorm > tol_gnorm) && (iter < max_iter) && ~stopping      
        ...
        if strcmp(step_alg, 'backtracking')
            rho = 1/2;
            c = 1e-4;
            [step, nfls] = backtracking_line_search(problem, d_old, w, rho, c);
        ...
        elseif strcmp(step_alg, 'strong_wolfe')
            c1 = 1e-4;
            c2 = 0.9;
            [step,~,~,nfls] = strong_wolfe_line_search(problem, d_old, w, c1, c2);
