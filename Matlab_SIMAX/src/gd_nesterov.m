function [w, infos] = gd_nesterov(w, problem, options)
...
%     end    
    if ~isfield(options, 'tol')
        ...
        tol_optgap = options.tol;
        tol_gnorm = options.tol;
    end
...
%     end 
    ...
    iter = 0;
    theta = 1;
    nf = 0; %--- number of function evaluation
    ...
    while (optgap > tol_optgap) && (gnorm > tol_gnorm) && (iter < max_iter)      
        ...
        if strcmp(step_alg, 'backtracking')
            rho = 1/2;
            c = 1e-4;
            [step, nfls] = backtracking_line_search(problem, -grad, w, rho, c);
        ...
        elseif strcmp(step_alg, 'strong_wolfe')
            c1 = 1e-4;
            c2 = 0.9;
            [step, ~, ~, nfls] = strong_wolfe_line_search(problem, -grad, w, c1, c2);
        ...
        nf = nf + nfls + 1;
        infos.func_eval = [infos.func_eval nf];
    end
