...
for i in range(3):
    ...
    file = f"cresults/loss_nltgcr{i}.pkl"
...
standard_errors = np.std(losses, axis=0)
...
for i in range(3):
    # Load the loss data from the pickle
    file = f"cresults/loss_mom{i}.pkl"
    with open(file, 'rb') as f:
        loss_data = pickle.load(f)
...
standard_errors3 = np.std(losses2, axis=0)
...
for i in range(2):
    # Load the loss data from the pickle
    file = f"cresults/loss_adam{i}.pkl"
    with open(file, 'rb') as f:
        loss_data = pickle.load(f)
...
standard_errors4 = np.std(losses2, axis=0)
...
plt.fill_between(range(len(mean_losses)), \
                 mean_losses - standard_errors, mean_losses + standard_errors, alpha=1)
...
plt.plot(range(len(mean_losses3)), mean_losses3, 'r-.', label='Nestrov')
plt.fill_between(range(len(mean_losses3)), \
                 mean_losses3 - standard_errors3, \
                    mean_losses3 + standard_errors3, color='red',alpha=0.1)
plt.plot(range(len(mean_losses4)), mean_losses4, 'g:', label='Adam')
plt.fill_between(range(len(mean_losses4)), \
                 mean_losses4 - standard_errors4, \
                    mean_losses4 + standard_errors4, color='green',alpha=0.2)
plt.xlabel('Epoch',fontsize=14)
plt.ylabel('Training Loss',fontsize=14)
plt.legend(fontsize=14)

plt.savefig('ctrainloss.png',dpi=200,bbox_inches='tight')
plt.show()
...
for i in range(3):
    # Load the loss data from the pickle
    file = f"cresults/acc_nltgcr{i}.pkl"
    with open(file, 'rb') as f:
        loss_data = pickle.load(f)
...
standard_errors = np.std(accs, axis=0)
...
standard_errors3 = np.std(accs, axis=0)

accs = []
# Loop through each pickle file
for i in range(2):
    # Load the loss data from the pickle
    file = f"cresults/acc_adam{i}.pkl"
    with open(file, 'rb') as f:
        loss_data = pickle.load(f)
    # Append the losses to the list of losses
    accs.append(loss_data)
...
plt.plot(range(len(mean_acc3)), mean_acc3, 'r-.', label='Nestrov')
plt.fill_between(range(len(mean_acc3)), \
                 mean_acc3 - standard_errors3, mean_acc3 + standard_errors3, color='red',alpha=0.5)
plt.plot(range(len(mean_acc4)), mean_acc4, 'g:', label='Adam')
plt.fill_between(range(len(mean_acc4)), \
                 mean_acc4 - standard_errors4, mean_acc4 + standard_errors4, color='green',alpha=0.5)
plt.xlabel('Epoch',fontsize=14)
plt.ylabel('Test Accuracy',fontsize=14)
# plt.ylim([0.6,0.9])
plt.legend(fontsize=14)
plt.savefig('cvalacc.png',dpi=200,bbox_inches='tight')
plt.show()